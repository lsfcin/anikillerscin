...
Textura Diablo::tRoda("Texturas/Carros/Chevette/roda.bmp",false);

Textura Diablo::tDiablo("Texturas/Carros/Diablo/diablo.bmp",false);

Diablo::Diablo(Ponto *centro,Vetor *n, Vetor *v, int cor): Carro(centro, n, v,10.0f, 0.01f, 0.4f,0.08f, 1.0f, 1.0f,0.5f,0.3f){
	this->tRoda.carregarBMP();
	Load3DS (&object,"Modelos/3DS/diablo.3ds");
	
	this->tDiablo.carregarBMP();
...
void Diablo::draw() {	
	...
	int l_index;
		...
		glTranslatef(this->centro->x,this->centro->y - 0.055,this->centro->z);
		...
		*temp = n->produtoVetorial(canonico);
		...
		glBindTexture(GL_TEXTURE_2D, tDiablo.ID); 
...
void Diablo::roda_origem(int raio) {
