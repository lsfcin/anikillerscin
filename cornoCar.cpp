...
Textura CornoCar::tRoda("Texturas/Carros/Chevette/roda.bmp",false);

Textura CornoCar::tCornoCar("Texturas/Carros/CornoCar/cornoCar.bmp",false);

CornoCar::CornoCar(Ponto *centro,Vetor *n, Vetor *v, int cor): Carro(centro, n, v,10.0f, 0.01f, 0.4f,0.08f, 1.0f, 1.0f,0.5f,0.3f){
	this->tRoda.carregarBMP();
	Load3DS (&object,"Modelos/3DS/cornoCar.3ds");
	
	this->tCornoCar.carregarBMP();
...
void CornoCar::draw() {	
	...
	int l_index;
		...
		glTranslatef(this->centro->x,this->centro->y + 0.14,this->centro->z);
		...
		*temp = n->produtoVetorial(canonico);
		...
		glScalef(0.00022f,0.00022f,0.00022f);
		glBindTexture(GL_TEXTURE_2D, tCornoCar.ID); 
...
void CornoCar::roda_origem(int raio) {
